...
- [◑] **P1.2 — Extension talks to the daemon, not the CLI.** ◑ Started — the daemon now exposes `/api/state` + `/api/verdict` (the endpoints the extension will call instead of shelling out); rewiring `vscode-extension/` is the remaining piece. Replace shell-outs with the daemon's socket/HTTP API for a live log stream and inline approvals. *Reuse:* `review.handle_api`. *Definition of done:* the timeline updates in real time without re-shelling `agentproof review --json`.
