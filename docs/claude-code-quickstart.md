# Tracewall for Claude Code — install & smoke test

Put Tracewall in front of Claude Code so every tool call (bash, file reads,
...
tracewall --help         # confirm the CLI is on your PATH
```
...
```bash
tracewall init           # creates .tracewall/ (a starter task contract)
...
Open Claude Code in this project and try these. You should see Tracewall act:
...
```bash
tracewall flow            # the ordered timeline, attributed to claude-code
tracewall review          # opens the allow/block review timeline in your browser
...
- **Fail-open:** if the hook ever errors, the action is **allowed** (with a note) —
  Tracewall can't brick your agent.
- **Hook not firing?** Restart Claude Code or run `/hooks`; confirm
  `.claude/settings.json` contains the `tracewall hook` commands; check `tracewall`
...
- **Remove it:** delete the Tracewall entries from `.claude/settings.json`.
